...
def smax(scores, label):
        ...
        return np.exp(scores[label])/np.exp(scores).sum(axis=1, keepdims=True)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        z2 = X.dot(W1) + b1  # (N, H)
        a2 = np.maximum(z2, 0)  # (N, H)
        z3 = a2.dot(W2) + b2  # (N, H) * (H, C) = (N, C)
        ...
        #############################################################################
        correct_class_mask = np.zeros_like(a3).astype(bool)  # (N, C)
        correct_class_mask[range(num_train), y] = True
        correct_class_scores = np.copy(a3)
        correct_class_scores -= 1
        correct_class_scores[~correct_class_mask] = 0

        incorret_class_scores = np.copy(a3)
        incorret_class_scores[correct_class_mask] = 0   # (N, C)
        delta3 = correct_class_scores + incorret_class_scores  # (N, C)
        grads['W2'] = a2.T.dot(delta3) / num_train + reg * W2 * 2  # (H, C)

        grads['b2'] = delta3.sum(axis=0) / num_train

        delta2 = delta3.dot(W2.T)  # (H, C) (N, C).T = (H, N)
        delta2[z2 < 0] = 0
        grads['W1'] = X.T.dot(delta2) / num_train + reg * W1 * 2  # d,n x n,h = d,h
        grads['b1'] = delta2.sum(axis=0) / num_train
